  ...
  class Reader
    class TermReader
      ...
      def active_term(id)
        ...
        term if cardinality(term)
      end

      def active_group(id)
        group = @registry.group(id)

        group if cardinality(group)
      rescue KeyError
      ...
      def cardinality(definition)
        @profile ? @profile.cardinality(definition) : definition.cardinality
      end
...
require 'eu_einvoice/reader/term_reader/unmapped'
require 'eu_einvoice/reader/term_reader/coercion'
