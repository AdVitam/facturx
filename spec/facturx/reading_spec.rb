...
RSpec.describe Facturx::Reading do
  ...
  it 'preserves diagnostic invariants through Data#with' do
    ...
    diagnostic = Facturx::Diagnostic.new(code: :invalid_value, message: 'Invalid').with(details:)
  ...
  it 'truncates diagnostic details at a valid character boundary' do
    diagnostic = Facturx::Diagnostic.new(code: :invalid_value, message: 'Invalid', details: { value: 'é' * 101 })
  ...
  it 'bounds diagnostic details at a valid character boundary' do
    detail = "#{'a' * 199}é"
    diagnostic = Facturx::Diagnostic.new(code: :invalid_value, message: 'Invalid', details: { value: detail })
  ...
  def diagnostic_input
    ...
    diagnostic = Facturx::Diagnostic.new(code: :invalid_value, term_id: :'BT-2', path: '/invoice/date',
