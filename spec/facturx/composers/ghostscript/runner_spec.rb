...
RSpec.describe Facturx::Composers::Ghostscript::Runner do
  it 'executes an argument vector without a shell' do
    ...
    expect(result.to_h).to eq(stdout: argument, stderr: '', exit_status: 0)
  end
  ...
  it 'wraps process startup failures' do
    expect { described_class.new.call(['/missing/facturx-ghostscript']) }
      .to raise_error(Facturx::CompositionError, /could not be executed/)
  end
  ...
  rescue Facturx::CompositionError => e
    e
  end
