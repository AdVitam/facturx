...
class WriterDocumentFactory
  class << self
    ...
    def payment
      Facturx::PaymentInstructions.new(
    ...
    def payment_card
      Facturx::PaymentCard.new(primary_account_number: '1234', holder_name: 'Buyer')
    end

    def direct_debit
      Facturx::DirectDebit.new(
    ...
    def adjustment_tax(present)
      Facturx::TaxBreakdown.new(category_code: 'S', rate: decimal('20')) if present
    end
