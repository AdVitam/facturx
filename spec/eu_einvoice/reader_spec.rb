...
RSpec.describe EuEinvoice::Reader do
  subject(:reader) { described_class.new(profile_resolver: cii_profile_detector) }
  ...
  describe 'typed EN16931 values' do
    ...
    it 'returns reading metadata and the exact source' do
      expect(reading).to have_attributes(
        profile: EuEinvoice::Profiles.fetch(:en16931), source_type: :xml, source: complete_en16931_xml, diagnostics: []
      )
    end
    ...
    it 'maps composite values' do
      expected = EuEinvoice::Quantity.new(value: BigDecimal('1'), unit_code: 'C62')
  ...
  it 'raises the typed profile error when requested' do
    ...
    expect { reader.call(xml, on_unknown_profile: :raise) }.to raise_error(EuEinvoice::UnknownProfileError)
  end
  ...
  it 'does not mix local and global delivery identifiers' do
    ...
    expect(identifiers).to eq([EuEinvoice::Identifier.new(value: 'LOCAL'),
                               EuEinvoice::Identifier.new(value: 'GLOBAL', scheme_id: '0088')])
  end
  ...
  it 'prefers a local delivery identifier while marking its global alternative' do
    ...
    expect([reading.document.delivery.location_identifier, unmapped_diagnostics?(reading)])
      .to eq([EuEinvoice::Identifier.new(value: 'LOCAL'), false])
  end
  ...
  it 'reads a BASIC credit transfer without the EN16931-only provider identifier' do
    ...
    expect(credit_transfer).to have_attributes(
      account_identifier: EuEinvoice::Identifier.new(value: 'FR761234'), provider_identifier: nil
    )
  end
  ...
  it 'maps a payment provider once for zero, one, or two credit transfers' do
    ...
    expect(readings.map { |reading| credit_transfer_attributes(reading) }).to eq(
      [[[], false], [[EuEinvoice::Identifier.new(value: 'BIC')], false],
       [[EuEinvoice::Identifier.new(value: 'BIC'), EuEinvoice::Identifier.new(value: 'BIC')], false]]
    )
  end
  ...
  it 'rejects non-CII XML' do
    expect { reader.call('<invoice/>'.b) }.to raise_error(EuEinvoice::InvalidXmlError)
  end
  ...
  def payment_without_means_expected
    direct_debit = EuEinvoice::DirectDebit.new(
      mandate_identifier: EuEinvoice::Identifier.new(value: 'MANDATE'),
      creditor_identifier: EuEinvoice::Identifier.new(value: 'CREDITOR')
    )
