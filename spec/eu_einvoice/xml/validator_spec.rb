...
RSpec.describe EuEinvoice::Xml::Validator do
  include XmlFixtureSupport

  subject(:validator) do
    described_class.new(profile_detector: cii_profile_detector, conformance_validator: cii_conformance)
  end
  ...
  it 'distinguishes a missing profile' do
    xml = xml_fixture(:minimum).sub(EuEinvoice::Profiles.fetch(:minimum).guideline_urn, '')
  ...
  it 'reports an unknown profile value' do
    xml = xml_fixture(:minimum).sub(EuEinvoice::Profiles.fetch(:minimum).guideline_urn, 'urn:example:unknown')
  ...
  it 'reports every XSD violation with the detected profile', :aggregate_failures do
    ...
    expect(report).to have_attributes(profile: EuEinvoice::Profiles.fetch(:minimum), invalid?: true)
  ...
  it 'returns optional conformance issues with the detected profile' do
    ...
    expect(report).to have_attributes(
      profile: EuEinvoice::Profiles.fetch(:minimum), issues: [issue], invalid?: true
    )
  end

  def validator_with(issue)
    conformance_validator = instance_double(EuEinvoice::Xml::ConformanceValidator, call: [issue],
                                                                                   steps: %i[xsd schematron])
    described_class.new(conformance_validator:, profile_detector: cii_profile_detector)
  end

  def schematron_issue
    EuEinvoice::Validation::Issue.new(
