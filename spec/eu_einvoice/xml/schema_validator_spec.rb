...
RSpec.describe EuEinvoice::Xml::SchemaValidator do
  include XmlFixtureSupport

  subject(:validator) { described_class.new(registry: EuEinvoice::Xml::SchemaRegistry.new) }
  ...
  let(:missing_profile) do
    EuEinvoice::Profile.new(id: :missing, guideline_urn: 'urn:example:missing', conformance_level: 'MISSING')
  end
  ...
  it 'loads a schema once per absolute path across validator instances' do
    ...
    2.times do
      validator_class.new(registry: EuEinvoice::Xml::SchemaRegistry.new).call(document: minimum_document,
  ...
  it 'wraps a missing schema without leaking Errno exceptions' do
    registry = instance_double(EuEinvoice::Xml::SchemaRegistry, fetch: '/missing/facturx.xsd')

    expect { described_class.new(registry:).call(document: minimum_document, profile: missing_profile) }
      .to raise_error(EuEinvoice::SchemaLoadError, 'Unable to load the Factur-X missing schema')
  end
  ...
  rescue EuEinvoice::XsdValidationError => e
    e
  end
  ...
  def validation_failure_validator
    ...
    validator_class.new(registry: EuEinvoice::Xml::SchemaRegistry.new)
  end
end
