...
RSpec.describe EuEinvoice::Composers::Ghostscript::VersionProbe do
  subject(:probe) { described_class.new(runner:) }

  let(:runner) { instance_double(EuEinvoice::Composers::Ghostscript::Runner) }
  ...
  it 'wraps execution failures as an unavailable version' do
    failure = EuEinvoice::CompositionError.new('failed', stderr: 'bounded diagnostic')
  ...
  def result(stdout)
    EuEinvoice::Subprocess::Result.new(
