...
RSpec.describe EuEinvoice::Pdf::Extractor do
  include PdfSupport

  subject(:extractor) { described_class.new(embedding: EuEinvoice::France::EMBEDDING) }
  ...
  it 'uses the configured filename and relationships without a country default' do
    embedding = EuEinvoice::Pdf::Embedding.new(**EuEinvoice::France::EMBEDDING.to_h,
                                             filename: 'invoice.xml', accepted_relationships: [:Supplement])
    pdf = build_pdf(catalog: '/AF [4 0 R]',
                    extra_objects: embedded_file_objects(xml, filename: 'invoice.xml', relationship: 'Supplement'))

    expect(described_class.new(embedding:).call(pdf)).to have_attributes(xml:, filename: 'invoice.xml')
  end
  ...
  it 'propagates invalid PDF errors' do
    expect { extractor.call('not a PDF') }.to raise_error(EuEinvoice::InvalidPdfError)
  end
  ...
  rescue EuEinvoice::ExtractionError => e
    e
  end
