...
class WriterDocumentFactory
  ...
  class << self
    ...
    def note
      EuEinvoice::Note.new(content: 'Note', subject_code: 'AAI')
    end

    def identifier(value, scheme_id = nil)
      EuEinvoice::Identifier.new(value:, scheme_id:)
    end

    def reference(id, name: nil, issue_date: nil)
      EuEinvoice::DocumentReference.new(id:, name:, issue_date:)
    end

    def delivery
      party = EuEinvoice::Party.new(name: 'Delivery party', address: address)
      EuEinvoice::Delivery.new(location_identifier: identifier('DELIVERY', '0088'), party:,
                               date: Date.new(2026, 9, 13))
    end

    def period
      EuEinvoice::Period.new(start_date: Date.new(2026, 9, 1), end_date: Date.new(2026, 9, 30))
    end

    def supporting_document
      EuEinvoice::SupportingDocument.new(
    ...
    def fixture_document(profile_id)
      EuEinvoice::Reader.new(profile_resolver: cii_profile_detector).call(File.binread("spec/fixtures/xml/#{profile_id}.xml")).document
    end
    ...
    def conforming_seller_address(profile_id)
      return EuEinvoice::Address.new(country_code: 'FR') if profile_id == :minimum
