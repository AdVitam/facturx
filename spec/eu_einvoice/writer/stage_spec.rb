...
RSpec.describe EuEinvoice::Writer::Stage do
  let(:profile) { EuEinvoice::Profiles.fetch(:minimum) }
  let(:tracker) { EuEinvoice::Writer::Tracker.new(profile:) }
  let(:context) do
    EuEinvoice::Writer::Context.new(document: EuEinvoice::Document.new, profile:, tracker:)
  end
    ...
    let(:stage) do
      ...
      Class.new(described_class) do
        define_method(:call) do
          within_group('BG-1', EuEinvoice::Note.new(content: 'forbidden'),
