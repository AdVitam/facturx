...
RSpec.describe EuEinvoice::Writer::Tracker, :aggregate_failures do
  subject(:tracker) { described_class.new(profile:) }

  let(:profile) do
    EuEinvoice::Profile.new(id: :en16931, guideline_urn: 'urn:example:en16931', conformance_level: 'EN 16931',
                            constraints: { 'BT-1' => '1..1', 'BT-2' => '0..1', 'BG-1' => '1..1', 'BG-2' => '0..1' })
  end

  it 'accepts a nil or matching document guideline' do
    document = EuEinvoice::Document.new(guideline_urn: profile.guideline_urn)
  ...
  it 'reports a profile mismatch' do
    ...
    expect(tracker.report.issues.first.to_h).to include(
  ...
  def term(id, cardinality)
    definition = EuEinvoice::TermDefinition.new(id, :document, :invoice_number, 'BG-0', :date, nil, cardinality)
    EuEinvoice::Term.new(definition, '/ram:ID', :date_102)
  end
  ...
  def group(id, cardinality)
    definition = EuEinvoice::GroupDefinition.new(id, :document, nil, nil, cardinality)
    EuEinvoice::Group.new(definition, '/ram:Group')
  end
  ...
  def formatting_error(term)
    EuEinvoice::Format.call('not a date', term:)
