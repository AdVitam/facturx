...
RSpec.describe EuEinvoice::Builders do
  describe EuEinvoice::Document do
    subject(:document) do
    ...
    it 'builds singular keyword associations' do
      expect(document.seller).to eq(EuEinvoice::Party.new(name: 'Seller'))
    end
    ...
    it 'builds collection items' do
      expect(document.lines).to eq([EuEinvoice::Line.new(id: '1')])
    end

    it 'accepts exact immutable association objects' do
      seller = EuEinvoice::Party.new(name: 'Seller')
    ...
    it 'rejects the wrong association model' do
      expect do
        described_class.build { |invoice| invoice.seller(EuEinvoice::Address.new(city: 'Paris')) }
      end.to raise_error(TypeError, /immutable EuEinvoice::Party/)
    end

    it 'rejects mutable uninitialized model objects' do
      expect do
        described_class.build { |invoice| invoice.seller(EuEinvoice::Party.allocate) }
      end.to raise_error(TypeError, /immutable EuEinvoice::Party/)
    end
    ...
    it 'makes singular helpers write-once' do
      first = EuEinvoice::Party.new(name: 'First')
      builder = EuEinvoice::Builders::DocumentBuilder.new(seller: first)
    ...
    it 'lets explicit singular setters replace an association' do
      builder = EuEinvoice::Builders::DocumentBuilder.new(seller: EuEinvoice::Party.new(name: 'First'))
      replacement = EuEinvoice::Party.new(name: 'Replacement')
    ...
    it 'appends collection helpers in call order' do
      builder = EuEinvoice::Builders::DocumentBuilder.new
      builder.line(id: '1')
      builder.line(EuEinvoice::Line.new(id: '2'))
    ...
    it 'lets explicit collection setters replace prior items' do
      builder = EuEinvoice::Builders::DocumentBuilder.new
      builder.line(id: '1')
      second = EuEinvoice::Line.new(id: '2')
    ...
    it 'does not retain mutable collection inputs' do
      lines = [EuEinvoice::Line.new(id: '1')]
      builder = EuEinvoice::Builders::DocumentBuilder.new(lines:)
      lines << EuEinvoice::Line.new(id: '2')
  ...
  it 'defines a named builder for every model in the association schema' do
    described_class::Schema::ASSOCIATIONS.each_key do |model|
      builder_name = "#{model.name.delete_prefix('EuEinvoice::')}Builder"
