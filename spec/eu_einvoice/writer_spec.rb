...
RSpec.describe EuEinvoice::Writer do
  subject(:writer) { described_class.new(conformance_validator: cii_conformance) }

  describe '#call' do
    context 'with no document guideline' do
      subject(:xml) { writer.call(document: minimum_document, profile: minimum_profile) }

      let(:minimum_profile) { EuEinvoice::Profiles.fetch(:minimum) }
      let(:minimum_document) do
        reading = EuEinvoice::Reader.new(profile_resolver: cii_profile_detector).call(File.binread('spec/fixtures/xml/minimum.xml'))
        address = EuEinvoice::Address.new(country_code: 'FR')
      ...
      it 'writes schema-valid XML that reads back to the document' do
        expect(EuEinvoice::Reader.new(profile_resolver: cii_profile_detector).call(xml).document).to eq(
    ...
    %i[minimum basic_wl basic en16931 extended].each do |profile_id|
      context "with every modeled #{profile_id} term" do
        let(:profile) { EuEinvoice::Profiles.fetch(profile_id) }
        let(:document) { WriterDocumentFactory.maximal_document(profile) }
        let(:xml) { writer.call(document:, profile:) }
        let(:reading) { EuEinvoice::Reader.new(profile_resolver: cii_profile_detector).call(xml) }
        ...
        it 'passes profile XSD validation' do
          expect(cii_validator.call(xml:)).to be_valid
        end
    ...
    context 'with repeated credit transfers' do
      ...
      let(:document) do
        ...
        second = first.with(
          account_identifier: EuEinvoice::Identifier.new(value: 'FR769999'),
          provider_identifier: EuEinvoice::Identifier.new(value: 'BIC-2')
        )
      ...
      it 'repeats the payment means code' do
        expect(payment_nodes.xpath('./ram:TypeCode', EuEinvoice::Xml::Namespaces::MAP).map(&:text))
          .to eq(%w[58 58])
      end

      it 'writes each payee account' do
        expect(payment_nodes.xpath('.//ram:PayeePartyCreditorFinancialAccount/ram:IBANID',
                                   EuEinvoice::Xml::Namespaces::MAP).map(&:text)).to eq(%w[FR761234 FR769999])
      end

      it 'writes each provider identifier' do
        expect(payment_nodes.xpath('.//ram:BICID', EuEinvoice::Xml::Namespaces::MAP).map(&:text))
          .to eq(%w[BIC BIC-2])
      end

      it 'writes the card only once' do
        expect(payment_nodes.xpath('.//ram:ApplicableTradeSettlementFinancialCard',
                                   EuEinvoice::Xml::Namespaces::MAP).size).to eq(1)
      end
      ...
      it 'reads each credit transfer back' do
        transfers = EuEinvoice::Reader.new(profile_resolver: cii_profile_detector).call(xml).document.payment.credit_transfers

        expect(transfers.map { |transfer| transfer.account_identifier.value }).to eq(%w[FR761234 FR769999])
      end

      it 'produces no reader diagnostic' do
        expect(EuEinvoice::Reader.new(profile_resolver: cii_profile_detector).call(xml).diagnostics).to be_empty
      end
    end
    ...
    context 'with global seller identifiers' do
      ...
      let(:identifiers) do
        [
          EuEinvoice::Identifier.new(value: 'SELLER-ONE', scheme_id: '0088'),
          EuEinvoice::Identifier.new(value: 'SELLER-TWO', scheme_id: '0060')
        ]
      end
      let(:document) do
        original = WriterDocumentFactory.complete_document(profile)
      ...
      it 'keeps each global identifier scheme on round trip' do
        reading = EuEinvoice::Reader.new(profile_resolver: cii_profile_detector).call(writer.call(document:, profile:))

        expect(reading.document.seller.identifiers).to eq(identifiers)
      end

      it 'reports a scheme-only global identifier' do
        identifier = EuEinvoice::Identifier.new(scheme_id: '0088')
    ...
    context 'with empty optional objects' do
      ...
      let(:document) do
        original = WriterDocumentFactory.complete_document(profile)
        seller = original.seller.with(contact: EuEinvoice::Contact.new)
        original.with(seller: seller, project_reference: EuEinvoice::DocumentReference.new)
      end
      ...
      it 'omits the empty contact wrapper' do
        expect(parsed.at_xpath('//ram:SellerTradeParty/ram:DefinedTradeContact',
                               EuEinvoice::Xml::Namespaces::MAP)).to be_nil
      end

      it 'omits the empty project wrapper' do
        expect(parsed.at_xpath('//ram:SpecifiedProcuringProject', EuEinvoice::Xml::Namespaces::MAP)).to be_nil
      end
    end
    ...
    context 'with unrepresentable optional values' do
      ...
      it 'reports a project reference without an identifier before XSD validation' do
        invalid_document = document.with(project_reference: EuEinvoice::DocumentReference.new(name: 'Project'))
      ...
      it 'uses the identifier as a project name when the supplied name is blank' do
        id_only_document = document.with(
          project_reference: EuEinvoice::DocumentReference.new(id: 'PROJECT', name: ' ')
        )
  ...
  describe '#validate' do
    ...
    it 'reports an explicit guideline mismatch' do
      basic_guideline = EuEinvoice::Profiles.fetch(:basic).guideline_urn
    ...
    context 'when the document is invalid' do
      ...
      rescue EuEinvoice::InvalidDocumentError => e
        e
      end

      it 'raises before XSD validation', :aggregate_failures do
        conformance_validator = instance_spy(EuEinvoice::Xml::ConformanceValidator)
        ...
        expect { local_writer.call(document: invalid_document, profile: minimum_profile) }
          .to raise_error(EuEinvoice::InvalidDocumentError)
    ...
    context 'when the generated XML violates the XSD' do
      let(:conformance_validator) { instance_double(EuEinvoice::Xml::ConformanceValidator) }
  ...
  describe 'stage coverage' do
    it 'covers every modeled term' do
      expect(described_class::MODELED_TERM_IDS).to match_array(EuEinvoice::Terms.all.map(&:id))
    end
  ...
  def raise_invalid_document_for(term_id, code: nil)
    raise_error(EuEinvoice::InvalidDocumentError) do |error|
  ...
  def writer_with_schematron_issue
    ...
    validator = instance_double(EuEinvoice::Xml::ConformanceValidator, call: [issue], steps: %i[xsd schematron])
    [issue, described_class.new(conformance_validator: validator)]
  end

  def xsd_error
    EuEinvoice::XsdValidationError.new(
  ...
  def project_reference_for(document)
    ...
    EuEinvoice::Reader.new(profile_resolver: cii_profile_detector).call(xml).document.project_reference
  end

  def missing_term_ids(xml, profile)
    parsed = Nokogiri::XML(xml)
    EuEinvoice::Terms.for_profile(profile).filter_map do |term|
      term.id if parsed.xpath(representative_xpath(term),
                              EuEinvoice::Xml::Namespaces::MAP).empty?
    end
  end

  def minimum_tax_currency_projection(xml)
    settlement = Nokogiri::XML(xml).at_xpath(
      '//ram:ApplicableHeaderTradeSettlement', EuEinvoice::Xml::Namespaces::MAP
    )
    namespaces = EuEinvoice::Xml::Namespaces::MAP
  ...
  def unrepresentable_gross_price_document
    line = document.lines.first.with(
      gross_price: EuEinvoice::Price.new(basis_quantity: document.lines.first.quantity)
    )
  ...
  def document_with_classification_qualifier
    classification = EuEinvoice::ProductClassification.new(list_id: 'STI')
  ...
  def document_with_quantity_unit
    line = document.lines.first.with(quantity: EuEinvoice::Quantity.new(unit_code: 'C62'))
    document.with(lines: [line])
  end

  def document_with_payee_identifier_scheme
    identifier = EuEinvoice::Identifier.new(scheme_id: '0002')
  ...
  def document_with_header_invoiced_object_scheme
    document.with(invoiced_object_identifier: EuEinvoice::Identifier.new(scheme_id: 'OBJ'))
  end

  def document_with_line_invoiced_object_scheme
    line = document.lines.first.with(invoiced_object_identifier: EuEinvoice::Identifier.new(scheme_id: 'OBJ'))
    document.with(lines: [line])
  end
  ...
  def payee_with_vat_document
    document.with(payee: document.payee.with(vat_identifier: EuEinvoice::Identifier.new(value: 'FR123')))
  end

  def ship_to_with_vat_document
    party = document.delivery.party.with(vat_identifier: EuEinvoice::Identifier.new(value: 'FR123'))
  ...
  def document_with_line_allowance_tax
    tax = EuEinvoice::TaxBreakdown.new(category_code: 'S', rate: BigDecimal('20'))
  ...
  def round_trip(document)
    EuEinvoice::Reader.new(profile_resolver: cii_profile_detector).call(writer.call(document:, profile:)).document
  end

  def document_without_reference_values
    line = document.lines.first.with(invoiced_object_identifier: EuEinvoice::Identifier.new)
    document.with(
      tender_or_lot_reference: EuEinvoice::DocumentReference.new,
      invoiced_object_identifier: EuEinvoice::Identifier.new,
      lines: [line]
    )
  end
